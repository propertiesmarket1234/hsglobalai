...
export default function SpatialDisplayClient() {
  ...
  const localizedDefPoints = sPage.definitionPoints || sPage.defPoints || definitionPoints;
  const localizedFeatures = (sPage.features || spatialFeatures).map((feat: any, idx: number) => ({
    ...spatialFeatures[idx],
    title: feat.title,
    description: feat.description,
    badge: feat.badge,
  }));
  const localizedIndustries = (sPage.industries || spatialIndustries).map((ind: any, idx: number) => ({
    ...spatialIndustries[idx],
    title: ind.title,
    category: ind.category,
    description: ind.description,
  }));
  const localizedBusinessValues = (sPage.businessValues || businessValues).map((val: any, idx: number) => ({
    ...businessValues[idx],
    title: val.title,
    description: val.description,
  }));
  const localizedComparison = sPage.comparisonRows || comparisonData;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 2,
        name: sPage.breadcrumbProducts || "Products",
        item: `https://www.hsglobalai.com${lPath("/products")}`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: sPage.breadcrumbCurrent || "Spatial Display",
        item: `https://www.hsglobalai.com${lPath("/products/spatial-display")}`,
      },
    ],
  };
    ...
    <main className="min-h-screen bg-black text-white selection:bg-cyan-500 selection:text-black">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-6 font-mono">
            <Link href={lPath("/products")} className="hover:text-cyan-400 transition-colors">
              {sPage.breadcrumbProducts || "Products"}
            </Link>
            <span>/</span>
            <span className="text-cyan-300 font-semibold">{sPage.breadcrumbCurrent || "Spatial Display"}</span>
          </div>
          ...
          >
            ...
            <div className="lg:col-span-7">
              ...
              <p className="mt-6 text-base leading-8 text-gray-300 sm:text-lg max-w-2xl">
                {sPage.heroDescription || "Naked-eye 3D spatial AI displays delivering glasses-free immersive visual experiences and real-time interactive avatars for high-impact commercial environments."}
                {sPage.heroInlineDesc1 || " Featuring an ultra-slim "}
                <strong>{sPage.heroInlineThick || "6 cm body thickness"}</strong>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-4">
                ...
                >
                  {sPage.ctaPrimaryBtn || (sPage.ctaPrimary ? `${sPage.ctaPrimary} →` : "Book Spatial Display Demo →")}
                </Link>
                <Link
                ...
                >
                  {sPage.ctaSecondaryBtn || sPage.ctaSecondary || "Download Spatial Datasheet (PDF)"}
                </Link>
              </div>
            </div>
              ...
              <div className="relative w-full max-w-[380px] overflow-hidden rounded-3xl border border-cyan-500/35 bg-neutral-950/90 p-3.5 backdrop-blur-xl shadow-[0_0_60px_rgba(6,182,212,0.25)]">
                ...
                <div className="relative aspect-[9/16] w-full overflow-hidden rounded-2xl bg-black border border-white/10 shadow-2xl">
                  <Image
                    ...
                    alt={sPage.imageAlt || "AI Spatial Display Volumetric Screen Showcase"}
                    fill
                    priority
                  ...
                  <div className="absolute bottom-4 left-4 right-4 z-10 rounded-xl border border-white/15 bg-black/80 p-3.5 backdrop-blur-md">
                    <div className="flex items-center justify-between text-xs font-mono">
                      <span className="text-cyan-400 font-semibold">{sPage.heroOverlayThickness || "● 6 CM THICKNESS"}</span>
                      <span className="text-amber-400 font-bold">4K ULTRA HD</span>
                    </div>
                    <p className="mt-1 text-[11px] leading-snug text-gray-300">
                      {sPage.heroOverlayCaption || "Spatial Display Touch Screen with 4K Ultra HD volumetric depth perception"}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="relative mx-auto max-w-5xl text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-purple-500/40 bg-purple-950/60 px-4 py-1.5 backdrop-blur-md mb-6 shadow-[0_0_20px_rgba(168,85,247,0.3)]">
            <span className="text-xs font-bold tracking-widest text-purple-300 uppercase">
              {sPage.hardwareHighlightBadge || "★ Confirmed Hardware Specifications"}
            </span>
          </div>
          ...
          <h3 className="mt-6 text-2xl sm:text-4xl md:text-5xl font-black uppercase tracking-wider text-white leading-tight">
            {sPage.hardwareHighlightTitle1 || "Spatial Display Touch Screen"}
            <br />
            <span className="text-gray-200">{sPage.hardwareHighlightTitle2 || "With "}</span>
          ...
          <p className="mt-6 mx-auto max-w-2xl text-base sm:text-lg leading-8 text-gray-300">
            {sPage.hardwareHighlightDesc}
          </p>

          <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <div className="rounded-2xl border border-purple-500/30 bg-purple-950/40 px-6 py-3.5 backdrop-blur-md">
              <p className="text-xs font-mono text-purple-300 uppercase">{sPage.cardProfileLabel || "Body Profile"}</p>
              <p className="text-xl font-bold text-white mt-0.5">{sPage.cardProfileVal || "6 cm Depth"}</p>
            </div>
            ...
            <div className="rounded-2xl border border-amber-500/30 bg-amber-950/40 px-6 py-3.5 backdrop-blur-md">
              ...
              <p className="text-xl font-bold text-amber-400 mt-0.5">{sPage.cardPanelVal || "4K Touch Screen"}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-xs font-bold tracking-[0.3em] text-cyan-400 font-mono uppercase">
              {sPage.defSectionBadge || "Product Definition"}
            </span>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {localizedDefPoints.map((pt: any, idx: number) => {
              const IconComp = definitionPoints[idx]?.icon || Eye;
              return (
                <motion.div
                  key={pt.title}
            ...
            <div className="lg:col-span-7">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/40 bg-cyan-950/80 px-4 py-1.5 backdrop-blur-md mb-6">
                ...
                <span className="text-xs font-bold tracking-widest text-cyan-300 uppercase font-mono">
                  {sPage.partnershipBadge || "Hardware & Software Partnership"}
                </span>
              </div>

              <h2 className="text-3xl font-extrabold tracking-tight text-white sm:text-5xl">
                {sPage.partnershipTitlePrefix || "Integrating Spatial Display with "}
                <span className="bg-gradient-to-r from-cyan-400 via-sky-300 to-cyan-500 bg-clip-text text-transparent">
                  {sPage.partnershipTitleHighlight || "DIHUAVA AI Digital Humans"}
                </span>
              </h2>

              <p className="mt-6 text-base leading-8 text-gray-300 sm:text-lg">
                {sPage.partnershipDesc1}
              </p>
              ...
              <div className="mt-6 rounded-2xl border border-cyan-500/40 bg-cyan-950/60 p-5 backdrop-blur-md">
                <span className="text-xs font-mono font-bold text-cyan-300 uppercase tracking-wider block mb-1">
                  {sPage.canonicalArchBadge || "Canonical AI Architecture"}
                </span>
                <p className="text-sm font-semibold text-white leading-relaxed">
                  {sPage.canonicalArchQuote}
                </p>
              </div>

              <p className="mt-6 text-sm text-gray-400 leading-relaxed">
                {sPage.partnershipDesc2}
              </p>
              ...
              <div className="mt-8">
                ...
                >
                  <span>{sPage.learnDihuavaLink || "Learn more about DIHUAVA AI Platform"}</span>
            ...
            <div className="lg:col-span-5">
              <div className="rounded-3xl border border-white/15 bg-black/80 p-8 backdrop-blur-xl shadow-2xl">
                <h3 className="text-xl font-bold text-white mb-6 border-b border-white/10 pb-4 font-mono">
                  {sPage.sysArchCardTitle || "System Architecture Division"}
                </h3>

                <div className="space-y-6">
                  <div className="rounded-2xl border border-cyan-500/30 bg-cyan-950/30 p-5">
                    <span className="text-xs font-mono font-bold text-cyan-400 uppercase">
                      {sPage.physLayerBadge || "Physical Layer (Hardware)"}
                    </span>
                  ...
                  <div className="rounded-2xl border border-sky-500/30 bg-sky-950/30 p-5">
                    <span className="text-xs font-mono font-bold text-sky-400 uppercase">
                      {sPage.intelLayerBadge || "Intelligence Layer (Software)"}
                    </span>
                    <h4 className="text-lg font-bold text-white mt-1">{sPage.intelLayerName || "DIHUAVA Platform"}</h4>
                    <ul className="mt-2 text-xs text-gray-300 space-y-1.5 font-mono">
                      {(sPage.intelBullets || ["• AI Digital Human Avatars", "• On-Device Speech & Local RAG", "• 29+ Global Languages", "• 100% Offline GPU Processing"]).map((b: string, i: number) => (
                        <li key={i}>{b}</li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="text-center">
            <span className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-400 font-mono">
              {sPage.featuresTitle || sPage.techOverviewBadge || "Technology Overview"}
            </span>
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-white md:text-5xl">
              {sPage.techOverviewHeading || "Next-generation spatial visuals for real-world spaces."}
            </h2>
          </div>
      ...
      <section className="relative overflow-hidden bg-black px-6 py-24 text-white border-b border-white/10">
        <div className="mx-auto max-w-7xl">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-xs font-bold tracking-[0.3em] text-cyan-400 font-mono uppercase">
              {sPage.targetDeploymentsBadge || "Target Deployments"}
            </span>
            <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-white sm:text-5xl">
              {sPage.targetDeploymentsTitle || "Spatial Display Across Commercial Sectors"}
            </h2>
            <p className="mt-4 text-base text-gray-300">
              {sPage.targetDeploymentsSubtitle}
            </p>
          </div>

          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {localizedIndustries.map((ind: any, idx: number) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-white/10">
                    ...
                    >
                      <span>{sPage.exploreIndustryBtn || "Explore Industry Solution"}</span>
      ...
      <section className="relative overflow-hidden bg-black px-6 py-24 text-white border-b border-white/10">
        <div className="mx-auto max-w-7xl">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-xs font-bold tracking-[0.3em] text-cyan-400 font-mono uppercase">
              {sPage.valueSectionBadge || "Commercial Intent & Value"}
            </span>
            <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-white sm:text-5xl">
              {sPage.valueSectionTitle || "What Business Problems Does Spatial Display Solve?"}
            </h2>
            <p className="mt-4 text-base text-gray-300">
              {sPage.valueSectionSubtitle}
            </p>
          </div>

          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {localizedBusinessValues.map((val: any, idx: number) => {
              const ValIcon = val.icon || Sparkles;
              return (
                <motion.div
                  key={val.title}
                  initial={{ opacity: 0, y: 20 }}
      ...
      <section className="relative overflow-hidden bg-neutral-950 px-6 py-24 text-white border-b border-white/10">
        <div className="mx-auto max-w-5xl">
          <div className="text-center mb-16">
            <span className="text-xs font-bold tracking-[0.3em] text-cyan-400 font-mono uppercase">
              {sPage.compSectionBadge || "Technology Comparison"}
            </span>
            <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-white sm:text-5xl">
              {sPage.compSectionTitle || "Spatial Display vs. Conventional Display"}
            </h2>
            <p className="mt-4 text-base text-gray-300">
              {sPage.compSectionSubtitle}
            </p>
          </div>
          ...
          <div className="overflow-hidden rounded-3xl border border-cyan-500/30 bg-black/80 shadow-2xl backdrop-blur-xl">
            <div className="grid grid-cols-12 bg-cyan-950/80 px-6 py-4 border-b border-cyan-500/30 font-mono text-xs font-bold uppercase tracking-wider text-cyan-300">
              <div className="col-span-4 sm:col-span-3">{sPage.tableHeaderCategory || "Feature Category"}</div>
              <div className="col-span-4 sm:col-span-4 text-gray-400">{sPage.tableHeaderConventional || "Conventional Display"}</div>
              <div className="col-span-4 sm:col-span-5 text-cyan-300">{sPage.tableHeaderSpatial || "Spatial Display"}</div>
            </div>

            <div className="divide-y divide-white/10">
              {localizedComparison.map((row: any, idx: number) => (
                <div
                  key={row.feature}
      ...
      <section className="relative overflow-hidden bg-black px-6 py-24 text-white border-b border-white/10">
        <div className="mx-auto max-w-5xl">
          <div className="text-center mb-12">
            <span className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-400 font-mono">
              {sPage.specsTitle || sPage.specsSectionBadge || "Technical Specifications"}
            </span>
            <h2 className="mt-2 text-3xl font-bold tracking-tight text-white sm:text-4xl">
              {sPage.specsSectionTitle || "Spatial Display Specifications"}
            </h2>
          </div>
      ...
      <CTA
        title={sPage.ctaTitle || "Bring 3D Spatial Displays"}
        highlightTitle={sPage.ctaHighlight || "To Your Customer Environment"}
        description={sPage.ctaDesc}
        primaryButtonText={sPage.ctaPrimaryText || sPage.ctaPrimary || "Book a Demo"}
        secondaryButtonText={sPage.ctaSecondaryText || sPage.ctaSecondary || "Download Datasheets (PDF)"}
